...
{
    public class SavedPromotionDayBeforeExpirationJob : IJob
    {
        public static JobKey Key = new JobKey(nameof(SavedPromotionDayBeforeExpirationJob), JobConstants.LoyaltyGroup);
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            Console.WriteLine($"[SavedPromotionDayBeforeExpirationJob] Started for PromotionId={promotionId}");

            if (promotionId == 0)
            {
                Console.WriteLine("[SavedPromotionDayBeforeExpirationJob] PromotionId is 0, exiting.");
                return;
            }

            // Get all saved promotions for this promotion that expire today (in local timezone)
            // Convert UTC now to local timezone to get the correct "today"
            ...
            Console.WriteLine(
                $"[SavedPromotionDayBeforeExpirationJob] PromotionId={promotionId}, TodayLocal={todayLocal:yyyy-MM-dd}, TodayUtc={todayUtc:o}, TomorrowUtc={tomorrowUtc:o}");
            ...
            var savedPromotions = await integrationDb.SavedPromotions
                ...
                .Where(x => x.PromotionId == promotionId
                    && x.Promotion.ExpiredAt >= todayUtc
                    && x.Promotion.ExpiredAt < tomorrowUtc
                    && x.Customer != null && !x.Customer.IsDeleted && !x.Customer.IsBlocked)
                .ToListAsync();

            Console.WriteLine(
                $"[SavedPromotionDayBeforeExpirationJob] PromotionId={promotionId}, SavedPromotionsCount={savedPromotions.Count}");

            foreach (var savedPromotion in savedPromotions)
            {
                // Determine if promotion expires today or tomorrow
                ...
                {
                    await ProcessEmailNotificationsAsync(savedPromotion, setting, htmlGenerationService, notificationService, expiresToday);

                    await ProcessPushNotificationsAsync(savedPromotion, setting, firebaseService, expiresToday);

                    if (!setting.EnablePushNotifications)
                    {
                        await ProcessSmsNotificationsAsync(savedPromotion, setting, notificationService, smsSettings, expiresToday);
                    }
                }
            }
        }
            ...
            bool expiresToday)
        {
            try
            {
                ...
                {
                    var body = await htmlGenerationService.GenerateSavedPromotionExpiresTomorrowEmail(
